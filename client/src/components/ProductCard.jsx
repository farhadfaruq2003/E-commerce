  ...
  return (
    product && (
      <div
        onClick={() => {
          navigate(
            `/product/${product.category.toLowerCase()}/${product?._id || product?.id}`
          );
          scrollTo(0, 0);
        }}
      ...
      >
        <div className="group cursor-pointer flex items-center justify-center px-2">
          <img
            ...
            src={getProductImage(product.name) || (product.image && (Array.isArray(product.image) ? product.image[0] : (typeof product.image === 'string' && product.image.startsWith('[') ? JSON.parse(product.image.replace(/'/g, '"'))[0] : product.image)))}
            alt={product.name}
          />
        </div>
